...
res = requests.get('http://news.tvbs.com.tw/news/realtime/politics/%d-%d-%d?cate=politics'%(today.year,today.month,today.day))
soup = BeautifulSoup(res.text)
for entry in soup.select('.realtime_news_content_titel1 a'):
    link = 'http://news.tvbs.com.tw'+ entry['href']
...
for link in nocrawled_new_links:
    ...
    soup = BeautifulSoup(res.text)
    try:
        date = soup.select('.under15')[0].text.replace('/','-')
        date = date+'+08'
    except Exception as e:
        date = None
        print 'error1',link
    try:
        title = soup.select('.titel26 strong')[0].text
    except Exception as e:
        title = ''
        print 'error2',link
    try:
        content = soup.select('.newsdetail-content')[0].text.strip().split('\n')[0]
    except Exception as e:
        content = ''
        print 'error3',link
    try:
    ...
    #存進 mongodb
    doc = {'author':'TVBS新聞','date':date,'title':title,'content':content,'href':href,'share_count':share_count,'like_count':like_count,'comments':comments,'keywords':keywords}
    collect.insert_one(doc)
    #存進 pgdb
    function.keywords_insert_pgdb(keywords)
    function.kw_relation_insert_pgdb(keywords)
    function.doc_insert_pgdb(doc,67,2) #doc,source,big_source
    function.doc_join_kw_insert_pgdb(keywords,href)
    function.daily_kw_insert_pgdb(keywords,date,67) #keywords,date,source_fk
    print '%d/%d'%(ind1,t1)
    ind1 += 1
