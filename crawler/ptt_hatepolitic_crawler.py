...
for i in range(30):
    link = 'https://www.ptt.cc/bbs/HatePolitics/index%s.html'%newest
    newest -= 1
    try:
        res = requests.get(link)
...
for link in nocrawled_art_links:
    ...
    function.doc_insert_pgdb(doc,5,1) #doc,source,big_source
    function.doc_join_kw_insert_pgdb(keywords,href)
    function.daily_kw_insert_pgdb(keywords,date,5) #keywords,date,source_fk
    '''
    print '%d/%d'%(ind1,t1)
    ind1 += 1
...
payload = {
'from':'/bbs/Gossiping/M.1451109114.A.C9E.html',
'yes':'yes'
}
rs = requests.session()
res = rs.post('https://www.ptt.cc/ask/over18',data=payload)
res = rs.get('https://www.ptt.cc/bbs/Gossiping/M.1451109114.A.C9E.html')
soup = BeautifulSoup(res.text)
source = []
for i in soup.select('.push-ipdatetime'):
    source.append(i.text.strip())
item = list(set(source))
data = {}
for i in item:
    data[i] = source.count(i)
k = sorted(data.items(), lambda x, y: cmp(x[1], y[1]),reverse=True)
for a,b in k:
    print a,b
